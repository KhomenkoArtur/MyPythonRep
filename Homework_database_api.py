...
class CSVParser:
    ...
    def parse_csv(self):
        try:
            ...
            print(f"File {self.file_path} hasn't found.")
        except Exception as e:
...
### 20240520 homework related t0 SQLITE DB
class DbSQLite:
    ...
    def insert(self, table_name, data):
        ...
        # SQL-query for dupes checking
        check_sql = f"SELECT 1 FROM {table_name} WHERE " + ' AND '.join([f"{col}=?" for col in data.keys()])
        ...
        try:
            # dupes check
            self.cursor.execute(check_sql, values)
            ...
            if result:
                ...
                self.conn.commit()
                print("Record has inserted succesfully.")
                return True
        except sqlite3.Error as e:
            print(f"Error: {e}")
            return False
...
class main():
    ...
    def run(self):
        ...
        while True:
            ...
            print("8. Use CSV file and insert data to SQLite DB")
...
#######################################################
##### NEWly created part of code related to SQLite db homework. Also another part present at 218 - 266 lines above
#######################################################
            elif choice == "8":

                file_path = input("Enter CSV file path for parsing.")

                db = DbSQLite('sqlite.db')

                csv_parser = CSVParser(file_path)
                csv_parser.parse_csv()
                #C:\Users\Artur_Khomenko\Desktop\input_directory\TEST.csv
                ...
                for row in rows:
                    ...
                    # SQL-query to create table in DB
                    create_table_sql = """
                               CREATE TABLE IF NOT EXISTS PRIVAT_ADD (
                                   id INTEGER PRIMARY KEY,
                                   TYPE TEXT NOT NULL,
                                   TEXT TEXT,
                                   expiration TEXT
                    ...
                    l_table_name = items[0][1]  # return first value of first item

                    print(l_table_name)

                    # insert data
                    if l_table_name == "news":
                        db.insert("NEWS_FEED", row)

                    elif l_table_name == "privat_ad":
                        db.insert("PRIVAT_ADD", row)
                    else:
                        print("Wrong table")
